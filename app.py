...
def _condition_mask(df: pd.DataFrame, cond: dict) -> "pd.Series":
    ...
    if match == "exact":
        ...
        return s.apply(lambda x: any(v.lower() in x.lower() for v in vals))
    elif match == "startswith":
        return s.apply(lambda x: any(x.lower().startswith(v.lower()) for v in vals))
    return pd.Series([True] * len(df), index=df.index)
...
def apply_filters(df: pd.DataFrame, filters: list) -> pd.DataFrame:
    for f in filters:
        ...
        for cond in conditions:
            combined = combined & _condition_mask(df, cond)
        df = df[combined] if mode == "include" else df[~combined]
            ...
            with st.expander(
                ...
                with tab_filt:
                    ...
                    for i, filt in enumerate(config["filters"]):
                        # Migrate old format (no conditions key)
                        ...
                        with st.container(border=True):
                            ...
                            with h2:
                                ...
                                if st.button("🗑 Filtro", key=f"{filename}__filt_{i}__rm", help="Eliminar filtro"):
                                    to_remove.append(i)
                                ...
                                with st.container(border=True):
                                    ...
                                    with cc1:
                                        ...
                                        new_col = st.selectbox(
                                            ...
                                            filt_opts,
                                            index=idx,
                                            key=f"{filename}__filt_{i}__cond_{ci}__col",
                                        )
                                        if new_col != prev_col:
                                            cond["values"] = []
                                        cond["column"] = new_col
                                    ...
                                    with cc2:
                                        ...
                                        new_match = st.selectbox(
                                            ...
                                            key=f"{filename}__filt_{i}__cond_{ci}__match",
                                        )
                                    ...
                                    with cc3:
                                        st.markdown("<br>", unsafe_allow_html=True)
                                        if len(filt["conditions"]) > 1 and st.button("🗑", key=f"{filename}__filt_{i}__cond_{ci}__rm", help="Eliminar condición"):
                                            cond_to_remove.append(ci)
                                    ...
                                    if cond["match"] == "exact":
                                        ...
                                        if col_name and src_col in df.columns:
                                            ...
                                            valid_default = [v for v in cond.get("values", []) if v in unique_vals]
                                            cond["values"] = st.multiselect(
                                                "Valores",
                                                options=unique_vals,
                                                default=valid_default,
                                                placeholder="Selecciona uno o más valores…",
                                                key=f"{filename}__filt_{i}__cond_{ci}__multi__{col_name}",
                                            )
                                            if not cond["values"]:
                                                st.caption("⚠️ Sin valores seleccionados — condición inactiva.")
                                        else:
                                            st.caption("← Selecciona una columna destino para ver los valores disponibles.")
                                    else:
                                        inp_key = f"{filename}__filt_{i}__cond_{ci}__chip_input"
                                        ...
                                        with inp_col:
                                            st.text_input(
                                                ...
                                                placeholder=f"Escribe un valor ({match_es.get(cond['match'], cond['match'])}) y haz clic en ＋",
                                                key=inp_key,
                                            )
                                        with btn_col:
                                            st.button(
                                                "＋",
                                                key=f"{filename}__filt_{i}__cond_{ci}__add_chip",
                                                on_click=_add_chip,
                                        ...
                                        if cond["values"]:
                                            st.markdown("<br>", unsafe_allow_html=True)
                                            per_row = 5
                                            chip_rows = [cond["values"][s: s + per_row] for s in range(0, len(cond["values"]), per_row)]
                                            for r_idx, row_vals in enumerate(chip_rows):
                                                chip_cols = st.columns(per_row)
                                                for j, val in enumerate(row_vals):
                                                    global_idx = r_idx * per_row + j
                                                    with chip_cols[j]:
                                                        if st.button(f"✕  {val}", key=f"{filename}__filt_{i}__cond_{ci}__chip_{global_idx}", use_container_width=True):
                                                            cond["values"].pop(global_idx)
                                        else:
                                            st.caption("⚠️ Sin valores agregados — condición inactiva.")

                            for ci in reversed(cond_to_remove):
                                filt["conditions"].pop(ci)

                            if st.button("＋ Agregar condición (AND)", key=f"{filename}__filt_{i}__add_cond"):
